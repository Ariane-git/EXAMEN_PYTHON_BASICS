...
#  - Capture df juste AVANT la cellule LabelEncoder 
#  - Capture df après la dernière cellule         
# ══════════════════════════════════════════════════════════════════════════════
@st.cache_data
def run_notebook(nb_path: str, csv_path: str):
    ...
    for cell in nb["cells"]:
        ...
        # Remplacer uniquement le nom du fichier CSV 
...
with st.sidebar:
    ...
    etude_filt = st.multiselect(
        "Niveau d'étude",
        etude_opts,
        default=etude_opts
    )

    age_min = int(df_viz["age"].min())
    age_max = int(df_viz["age"].max())
...
if page == "📋 Données":
    ...
    if missing_cols.empty:
        st.success("nettoyage du notebook appliqué avec succès.")
    else:
